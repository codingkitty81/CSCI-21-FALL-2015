 ...
 * This program utilizes a class to gather the specs for a spaceship, and
 * then displays the specs.
 *
 ...
 *
 * Resources:
 * http://stackoverflow.com/questions/5113221/decimal-points-with-stdstringstream
 * 
 */
#include <cassert>
#include <iomanip>
...
string Spaceship::toString () const {
	...
	stringstream shipInfo;
	shipInfo.precision(2);
	shipInfo << fixed << name << ", top speed: warp " << topSpeed << ", fuel source: " << fuelSource << ", crew capacity: " << crewCapacity;
	return shipInfo.str();
