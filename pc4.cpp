 ...
 * This program streams input of varying types into a string and outputs it in
 * a desired order. It will evaluate whether a string is only one character
 * long and will return that character. Then it will give the numeric value as
 * an integer and as a double. Lastly, it evaluates the first letter input
...
char stringToChar (string value) {
	...
	if (value.length() == 1) {
		char *charValue = (char*)value.c_str();
		return(charValue[0]);
	}
	else {
		return('\0');
