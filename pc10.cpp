 ...
 * This program uses classes to convert either temperature or distance
 * either to metric or Imperial depending on users choices.
 *
...
float Converter::convertTemperature (float temp, char targetUnits) {
	...
	switch (targetUnits) {
		case 'I':
			return Converter::celsiusToFahrenheit(temp);
			break;
		case 'M':
...
float Converter::convertDistance (float distance, char targetUnits) {
	...
	switch (targetUnits) {
		case 'I':
			return Converter::feetToMeters(distance);
			break;
		case 'M':
...
float Converter::metersToFeet (float distance) {
	float newDistance = distance / 3.2808399;
	return newDistance;
}
