 ...
 * This program takes arrays and sorts them using either the bubble
 * sort method or the selection method. Once the desired array is
 * sorted it is compared to another array that is already in
 * accending order and will return true if they match, otherwise
 * false is returned to indicate sorting failure.
 *
...
void selectionSort (int theArray[], unsigned int size) {
	...
	for (int i = 0; i < size; i++) {
		...
		for (int j = i + 1; j < size; j++) {
			if (theArray[j] < theArray[subInt]) {
				subInt = j;
			}
		}
		swapValues(theArray, i, subInt);
	}
}
